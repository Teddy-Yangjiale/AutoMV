...
Studio 的六种逐句动效会原样写入项目 JSON，并由 libass 在成片中重建：

- `cinematic`：纵向缓慢上浮，长淡入淡出，适合抒情和叙事。
- `float`：水平方向连续漂移，相邻句交替进入方向。
- `punch`：快速缩放撞入，适合鼓点清晰的流行段落。
- `handwritten`：轻微位移与旋转，适合手写、粉笔和日记视觉。
- `neon`：从模糊辉光聚焦成像，适合夜景与电子氛围。
- `minimal`：低位移淡入淡出，适合克制的黑白画面。

默认 `displayMode=single`，成片只保留当前句并让上一句沿同一运动轴离场；需要导唱或校对时可切换为 `stack` 显示淡化的前后文。
